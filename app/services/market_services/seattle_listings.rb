module MarketServices
  class SeattleListings
    attr_reader :market

    def initialize(market = nil)
      @market = market || Market.find_by_name("Seattle")
    end
    ...
    def parse(response)
      ...
      rows.each do |row|
        mls = row.search("td.d130m6").text
        price = row.search("td.d130m14").text.gsub(",", "").gsub("$", "").to_f
        address = row.search("td.d130m9").text
        city = row.search("td.d130m13").text
        deep_comps = ZillowService::GetDeepComps.call(address, "#{city}, WA")
        ...
        if deep_comps.present?
          ...
          listing.city = city
          listing.added_date = format_time(row.search("td.d130m18").text)
          listing.chg_type = nil
          listing.sq_ft = row.search("td.d130m17").text.gsub(",", "").to_f
