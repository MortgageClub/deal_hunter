module MarketServices
  class FortLauderdaleListings
    attr_reader :market

    def initialize(market = nil)
      @market = market || Market.find_by_name("Fort Lauderdale")
    end
    ...
    def parse(response)
      ...
      rows.each do |row|
        ...
        price = row.search("td.d5m19").text.gsub(",", "").gsub("$", "").to_f
        address = row.search("td.d5m11 .field").text
        city = row.search("span.d5m12").text + ", " + row.search("td.d5m13 .field").text
        ...
        if deep_comps.present?
          ...
          listing.sq_ft = row.search("span.d5m18").text.gsub(",", "").to_f
          listing.year_built = nil
          listing.bed_rooms = row.search("td.d5m14 .field").first.text.to_i
          listing.bath_rooms = row.search("td.d5m16 .field").text.to_f
