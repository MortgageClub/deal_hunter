module MarketServices
  class MiamiListings
    ...
    def initialize(market = nil)
      @market = market || Market.find_by_name("Miami")
    end
    ...
    def parse(response)
      ...
      rows.each do |row|
        ...
        address = row.search("td.d5m11 span").text
        city = row.search("span.d5m12").text
        city = "#{city}, FL"
        ...
        if deep_comps.present?
          ...
          listing.bed_rooms = row.search("span.d5m15").text.to_i
          listing.bath_rooms = row.search("td.d5m16 span").text.to_f
          listing.lot_sz = nil
          listing.price = price
